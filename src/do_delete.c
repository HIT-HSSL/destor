...
struct timeval rt0, rt1, wt0, wt1;
...
static void* read_recipe_for_deletion(void *arg) {
    int* startJIDPtr = (int*)arg;

    GHashTable * involved_containers = g_hash_table_new_full(g_int64_hash, g_int64_equal, free, NULL);

    {
        uint64_t involved_container = 0;

        struct backupVersion* bv = open_backup_version(*startJIDPtr);
        int i, j, k;
        for (i = 0; i < bv->number_of_files; i++) {
            ...
            for (j = 0; j < r->chunknum; j++) {
                ...
                if(!g_hash_table_contains(involved_containers, &cp->id)){
                    uint64_t* id = (uint64_t*)malloc(sizeof(uint64_t));
                    *id = cp->id;
                    g_hash_table_insert(involved_containers, id, NULL);
                    involved_container++;
                }
    ...
    for(int m=1; m<=destor.backup_retention_time; m++){
        assert(backup_version_exists(*startJIDPtr + m));
        struct backupVersion* bv = open_backup_version(*startJIDPtr + m);
        ...
        for (i = 0; i < bv->number_of_files; i++) {
            ...
            printf("total involved chunks:%lu\n", count);
    ...
    g_hash_table_destroy(involved_containers);
...
struct GCHashEntry{
    uint64_t cid;
    GHashTable* chunk_queue;
};

void destructor(gpointer ptr){
    struct GCHashEntry* eptr = (struct GCHashEntry*)ptr;
    g_hash_table_destroy(eptr->chunk_queue);
    free(ptr);
}

gboolean g_fp_equal(fingerprint * l, fingerprint * r){
    return !memcmp(l, r, sizeof(fingerprint));
}

guint g_fp_hash(fingerprint * fp){
    guint* hp = (guint*)fp;
    return *hp;
}
...
static void* gether_fingerprint_for_deletion(void *arg) {
    ...
    while(c = sync_queue_pop(delete_recipe_queue)){
        ...
        if(gcHashEntry == NULL){
            ...
            entry->cid = c->id;
            entry->chunk_queue = g_hash_table_new_full(g_fp_hash, g_fp_equal, free, NULL);

            fingerprint* fp = (fingerprint*)malloc(sizeof(fingerprint));
            memcpy(fp, &c->fp, sizeof(fingerprint));
            g_hash_table_insert(entry->chunk_queue, fp, NULL);

            g_hash_table_insert(global_gc_HashTable, &entry->cid, entry);
        }else{
            fingerprint* fp = (fingerprint*)malloc(sizeof(fingerprint));
            memcpy(fp, &c->fp, sizeof(fingerprint));
            g_hash_table_insert(gcHashEntry->chunk_queue, fp, NULL);
        }
        count++;
        free_chunk(c);
    }
    printf("recipe : %lu chunks\n", count);
    return NULL;
}
...
uint64_t involved_chunks;
uint64_t con_counter;
uint64_t migrate_counter;
uint64_t migrate_size;
uint64_t skip_counter;
uint64_t skip_chunks;

struct chunk_filter_arg{
    GHashTable * filter;
    GHashTable * migrateList;
};

void chunk_filter(gpointer key, gpointer value, gpointer user_data){
    fingerprint * fp = (fingerprint*)key;
    struct chunk_filter_arg* cfa = (struct chunk_filter_arg*)user_data;
    if(g_hash_table_contains(cfa->filter, key)){
        struct metaEntry* me = (struct metaEntry*)malloc(sizeof(struct metaEntry));
        memcpy(me, value, sizeof(struct metaEntry));
        g_hash_table_insert(cfa->migrateList, &me->fp, me);
    }
}
...
void read_container_filter(gpointer key, gpointer value, gpointer user_data){
    ...
    printf("container %lu, total chunks %d, alive chunks %d\n", e->cid, con->meta.chunk_num, g_hash_table_size(e->chunk_queue));
    con_counter++;
    involved_chunks += con->meta.chunk_num;

    if(g_hash_table_size(e->chunk_queue) == con->meta.chunk_num){
        skip_counter++;
        skip_chunks += con->meta.chunk_num;
    }else{
        GHashTable * tempTable = g_hash_table_new_full(g_fp_hash, g_fp_equal, NULL, free);
        struct chunk_filter_arg cfa;
        cfa.filter = e->chunk_queue;
        cfa.migrateList = tempTable;

        container_meta_foreach(&con->meta, delete_an_entry, &e->cid);
//        g_hash_table_foreach(e->chunk_queue, chunk_filter, con->meta.map);
//        g_hash_table_foreach(con->meta.map, chunk_migrate, con);
        g_hash_table_foreach(con->meta.map, chunk_filter, &cfa);
        g_hash_table_foreach(cfa.migrateList, chunk_migrate, con);
        g_hash_table_destroy(tempTable);
    }
...
static void* load_container_for_deletion(void *arg) {
    ...
    SET_CHUNK(c, CHUNK_FILE_START);
    sync_queue_push(migrate_data_queue, c);
    con_counter = 0, migrate_counter = 0, migrate_size = 0, involved_chunks = 0, skip_counter = 0, skip_chunks = 0, readDuration = 0, jcr.write_time = 0;

    g_hash_table_foreach(global_gc_HashTable, read_container_filter, NULL);
    printf("%lu containers (%lu chunks) involved (%lu containers (%lu chunks) skipped), %lu chunks (%lu bytes) migrated\n", con_counter, involved_chunks, skip_counter, skip_chunks, migrate_counter, migrate_size);
...
static void* write_container_for_deletion(void *arg) {
    ...
    while(c = sync_queue_pop(migrate_data_queue)){
        ...
        if (CHECK_CHUNK(c, CHUNK_FILE_END)) {
            ...
            index_update(features, get_container_id(con));
...
void do_delete(int jobid) {
    ...
    endFlag = false;
    pthread_create(&read_t, NULL, read_recipe_for_deletion, &jobid);
    ...
    printf("gc duration : %lu us\n", duration);
    printf("read duration : %lu us\n", readDuration);
    printf("write duration : %f us\n", jcr.write_time);
}
